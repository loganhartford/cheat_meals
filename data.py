...
def get_coordinates_from_address(address):
    ...
    params = {"address": address, "key": PLACES_API_KEY}
...
def generate_distance_col(restaurant_locations_df, lat, lng):
    ...
    return restaurant_locations_df.apply(
        lambda df: find_distance_between_coordinates(
...
def keep_only_closest_location(restaurant_locations_df):
    ...
    return restaurant_locations_df.drop_duplicates(
...
def clean_restaurant_name(restaurant_locations_df):
    ...
    return restaurant_locations_df.apply(
        lambda df: df["Name"].replace("'", "").lower(), axis=1
    )
...
def calculate_cheat_score(total_cals, total_fat, sodium, sugar):
    ...
    # Fat %
    m, b = get_linear_constants(
        (PERCENT_FAT_MIN, 0), (PERCENT_FAT_MAX, PERCENT_FAT_WEIGHT)
    )
    fat_score = min(
        m * total_fat * CALORIES_PER_GRAM_OF_FAT / total_cals + b,
        PERCENT_FAT_WEIGHT + 1,
    )
    # Sodium
    ...
    cheat_score = round(
        min(calorie_score + fat_score + sodium_score + sugar_score, MAX_CHEAT_SCORE), 2
    )
    return cheat_score
...
def create_cheat_score_column(meal_items_df):
    ...
    return meal_items_df.apply(
        lambda df: calculate_cheat_score(
            df["calories"], df["total_fat"], df["sodium"], df["sugar"]
        ),
        axis=1,
    )
...
def remove_restaurants_without_meals(meal_items_df, restaurant_locations_df):
    ...
    return restaurant_locations_df[
        restaurant_locations_df["Name"].isin(restaurants)
    ].reset_index(drop=True)
...
def create_distance_and_location_column(meal_items_df, restaurant_locations_df):
    ...
    address_col = meal_items_df.apply(
        lambda df: get_address(df["restaurant"], restaurant_locations_df), axis=1
    ).values.tolist()
    distance_col = meal_items_df.apply(
        lambda df: get_distance(df["restaurant"], restaurant_locations_df), axis=1
    ).values.tolist()
...
def get_cheat_meals(
    ...
    ) = create_distance_and_location_column(menu_items_df, rest_locs_df)
